    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {loan.status === 'open' ? (
            <Badge className="badge-warning text-sm">Open</Badge>
          ) : (
            <Badge className="badge-success text-sm">Paid</Badge>
          )}
        </div>
      </div>
      ...
      {loan.fraud_flags?.length > 0 && (
        <div className="flex gap-2 flex-wrap">
          {loan.fraud_flags.includes('QUICK_CLOSE') && (
            <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-amber-500/10 border border-amber-500/20 animate-shake">
              <Clock className="w-4 h-4 text-amber-500" />
              <span className="text-sm text-amber-500 font-medium">Quick-Close Alert: Loan paid same day as created</span>
            </div>
          )}
          {loan.fraud_flags.includes('DUPLICATE_CUSTOMER') && (
            <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-purple-500/10 border border-purple-500/20 animate-shake">
              <AlertTriangle className="w-4 h-4 text-purple-500" />
              <span className="text-sm text-purple-500 font-medium">Duplicate Customer: Multiple loans for this customer</span>
            </div>
          )}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="border-border" data-testid="customer-info-card">
          ...
          <CardContent className="space-y-3">
            ...
            <div>
              ...
              <p className="font-mono text-sm">{canViewFullId ? loan.customer_id_number : loan.customer_id_number_masked}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Mandate ID</p>
              <p className="font-mono text-sm">{loan.mandate_id}</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-border" data-testid="loan-summary-card">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg font-heading flex items-center gap-2">
              <CreditCard className="w-5 h-5 text-emerald-500" strokeWidth={1.5} />
              Loan Summary
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <p className="font-mono font-medium">R{loan.principal_amount.toFixed(2)}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Interest (40%)</p>
                <p className="font-mono font-medium">R{(loan.principal_amount * 0.4).toFixed(2)}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Service Fee</p>
                <p className="font-mono font-medium">R{loan.service_fee.toFixed(2)}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Total Repayable</p>
                <p className="font-mono font-medium text-emerald-500">R{loan.total_repayable.toFixed(2)}</p>
              </div>
            </div>
            <div className="pt-2 border-t border-border">
              <p className="text-sm text-muted-foreground">Outstanding Balance</p>
              <p className="font-mono text-xl font-bold text-amber-500">R{loan.outstanding_balance.toFixed(2)}</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-border" data-testid="loan-info-card">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg font-heading flex items-center gap-2">
              <CalendarDays className="w-5 h-5 text-emerald-500" strokeWidth={1.5} />
              Loan Info
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div>
              ...
              <p className="font-medium">{new Date(loan.loan_date).toLocaleDateString('en-ZA', { dateStyle: 'long' })}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Repayment Plan</p>
              <p className="font-medium">{planNames[loan.repayment_plan_code]}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Created By</p>
              <p className="font-medium">{loan.created_by_name}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Progress</p>
              <p className="font-medium">{paidPayments} of {totalPayments} payments</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-border" data-testid="payment-schedule-card">
        <CardHeader>
          <CardTitle className="text-lg font-heading flex items-center gap-2">
            <Banknote className="w-5 h-5 text-emerald-500" strokeWidth={1.5} />
            Payment Schedule
          </CardTitle>
          <CardDescription>
                          ...
                          <AlertDialogContent className="bg-card border-border">
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                ...
                                <br /><br />
                                <strong className="text-amber-500">This action cannot be undone.</strong>
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
